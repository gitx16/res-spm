    ...
    var FormUpload = FormWidget.extend({
        ...
        setup: function() {
            ...
            if (value.fileName) {
                ...
                this.$(".file-title").text(fileName || "未选择内容");
            }
        },
        bindAjaxUpload: function() {
            ...
            var conf = {
                ...
                onComplete: function(btn, response) {
                    ...
                    self.$(".file-title").text(filename);
                    self.fileInfo = sco;
                    self.trigger("complete");
                }
            };
            this.$(".file-choice").ajaxUpload(conf);
        },
        getValue: function() {
...
define("value/form-upload/1.0.0/upload-debug.handlebars", [ "gallery/handlebars/1.0.2/runtime-debug" ], function(require, exports, module) {
    ...
    module.exports = template(function(Handlebars, depth0, helpers, partials, data) {
        ...
        buffer += escapeExpression(stack2) + '" class="btn btn-default file-choice">选择文件</label>\n        <span class="file-title">未选择内容</span>\n    </div>\n</div>';
        return buffer;
    });
});
